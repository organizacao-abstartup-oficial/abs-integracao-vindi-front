...
export default function FormRenovacao() {
  ...
  async function handleSubscription(cnpj){
    axios.get(`http://localhost:8080/subscription/subs/${cnpj.replace(/\D/g, '')}`).then(response => {
  ...
  function getSteps() {
  return ['CNPJ', 'Informações', 'Pagamento', 'Confirmação'];
  }
    ...
    switch (step) {
      case 0:
        ...
        <TextField
          ...
          required={true}
          id="name"
          value={consumer.metadata.nome_pessoa_fisica}
  ...
  const Thanks = (
    <div>
      ...
      />
        ...
        <p>O Plano Contratado é o: <b>Impact</b></p>
        ...
        {allStepsCompleted() ? (
          <div>
            ...
            />
              ...
              <p>O Plano Contratado é o: <b>Impact</b>{ idConsumer ? '' : '' }</p>
